...
public class MutantRest {
	...
	@ApiOperation(value = "Permite al mutante Magneto validar si eres un mutante para su combate contra los x-men", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE, httpMethod = "POST")
	@ApiResponses(value = {
	...
	public Response isMutant(
			@ApiParam(value = "Objeto en formato JSON con el DnaDto", name = "dna", required = true, type = "DnaDto") @RequestBody DnaDto dna) {
		return this.serviceMuntant.validateMutant(dna);
	}

	@ApiOperation(value = "Permite al mutante Magneto consultar todos ADN que a analizado hasta ahora", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE, httpMethod = "GET")
	@ApiResponses(value = {
	...
	@ApiOperation(value = "Permite al mutante Magneto consultar sus estadisticas sobre los mutantes para su combate contra los x-men", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE, httpMethod = "GET")
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "La consulta se realiza de manera exitosa", response = ResponseEntity.class),
			@ApiResponse(code = 500, message = "Ocurre un error dentro del servidor, el cual se puede generar por problemas en el acceso a recursos, consultas de elementos no existentes o errores inesperados") })
	@GetMapping("/stats")
